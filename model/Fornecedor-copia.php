    ...
    public function inserir()
{
    try {
        ...
        $executar->bindValue(4, mb_strtoupper($this->funcao));
        return $executar->execute() == 1 ? true : false;
    } catch (PDOException $erro) {
        echo $erro->getMessage();
    }
...
public function alterar()
{
    try {
        ...
        $sql = "UPDATE {$this->tabela}
                SET nome = ?, empresa = ?, funcao = ?
                WHERE id = ?";

        $executar = $this->conn->prepare($sql);
        $executar->bindValue(1, mb_strtoupper($this->nome));
        $executar->bindValue(2, mb_strtoupper($this->empresa));
        $executar->bindValue(3, mb_strtoupper($this->funcao));
